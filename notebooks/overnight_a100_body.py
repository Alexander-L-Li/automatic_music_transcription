...
if not root.exists():
    raise RuntimeError("Expected /content/frozen_prior_amt from the fresh-cell bootstrap wrapper.")
os.chdir(root)

candidate_srcs = [root / "src"]
candidate_srcs.extend(p.parent.parent for p in Path("/content").glob("**/frozen_prior_amt/config.py"))
candidate_srcs = [p for i, p in enumerate(candidate_srcs) if p.exists() and p not in candidate_srcs[:i]]
if not candidate_srcs:
    raise RuntimeError("Could not find frozen_prior_amt source under /content.")
for name in list(sys.modules):
    if name == "frozen_prior_amt" or name.startswith("frozen_prior_amt."):
        del sys.modules[name]
for src in reversed(candidate_srcs):
    sys.path.insert(0, str(src))
print("using source paths:", [str(p) for p in candidate_srcs])
...
# Locked from the validation rescue and previous held-out test confirmation.
SEEDED_PRIOR_SCALE = 0.25
SEEDED_START_FRAC = 0.75
SEEDED_THRESHOLD = 0.4

# A100 overnight profile. Keep batch at 64 because it was the best known stable
# setting before GPU-resident sampling; use more steps/pieces for scale.
...
# Cache a tiny audio/alignment sanity sample from the prewritten stratified
# subset. Do not call the CLI `data-sanity` command here: that command creates
# a new shortest-first subset and would overwrite this named stratified subset.
...
baseline_ckpt = latest("baseline-*/baseline.pt", since=run_started)
...
prior_ckpt = latest("midi-prior-*/midi_prior.pt", since=run_started)
...
control_ckpt = latest("control-*/control_branch.pt", since=run_started)

print("\nnew baseline:", baseline_ckpt)
print("new prior:", prior_ckpt)
print("new control:", control_ckpt)
...
@torch.no_grad()
def run_locked_stratified_test():
    ...
    for stage, stage_rows in rows.items():
        ...
        elif stage == "pure_control":
            threshold = pure_control_threshold
        else:
            threshold = SEEDED_THRESHOLD
        summary_rows.append({
            "stage": stage,
            "threshold": threshold,
            "windows": len(stage_rows),
            **mean_metrics(stage_rows),
        })
    return pd.DataFrame(summary_rows).sort_values("onset_f1", ascending=False), saved_examples

summary, examples = run_locked_stratified_test()
