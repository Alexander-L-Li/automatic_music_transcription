# Supervised refiner load-bearing ablation for the post-overnight checkpoint set.
#
# Run this as one Colab A100 cell after the repo payload has been unpacked at
# /content/frozen_prior_amt. It trains a one-shot supervised refiner, re-runs the
# frozen-prior and no-prior distribution-matched checkpoints through the same
# evaluator, writes the result table/CIs/figure/interpretation, and saves a
# reload-and-eval snippet.
...
RUN_NAME = "supervised_refiner_ablation_20260506"
...
from frozen_prior_amt.training import configure_torch_for_device, train_supervised_refiner
...
def collect_predictions(dataset, split_name: str) -> list[dict[str, object]]:
    ...
    for batch in tqdm(loader_for(dataset, EVAL_BATCH), desc=f"{split_name} shared eval", mininterval=10.0, maxinterval=30.0):
        ...
        with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=device.type == "cuda"):
            ...
            supervised_probs = torch.sigmoid(refiner_eval(baseline_probs, batch["x"])).float()
...
def threshold_sweep(records: list[dict[str, object]], split_name: str) -> pd.DataFrame:
    rows = []
    for variant in ["baseline_raw", "supervised_refiner", "no_prior_distmatched", "frozen_prior_distmatched"]:
...
def bootstrap_deltas(window_df: pd.DataFrame) -> dict[str, dict[str, float]]:
    ...
    pairs = {
        "supervised_refiner_minus_baseline_raw": ("supervised_refiner", "baseline_raw"),
        "supervised_refiner_minus_no_prior_distmatched": ("supervised_refiner", "no_prior_distmatched"),
        "supervised_refiner_minus_frozen_prior_distmatched": ("supervised_refiner", "frozen_prior_distmatched"),
    }
    out = {}
...
def interpretation(results: pd.DataFrame, cis: dict[str, dict[str, float]]) -> str:
    ...
    d_nop = cis["supervised_refiner_minus_no_prior_distmatched"]
    d_frozen = cis["supervised_refiner_minus_frozen_prior_distmatched"]
    lines = [
        f"The supervised refiner reached onset F1 {sup.onset_f1:.3f} at threshold {sup.threshold:.2f}.",
        f"The no-prior distribution-matched diffusion refiner reached {nop.onset_f1:.3f}, so supervised minus no-prior is {sup.onset_f1 - nop.onset_f1:+.3f} by the table and bootstrap median {d_nop['median']:+.3f} with 95% CI [{d_nop['ci_low']:+.3f}, {d_nop['ci_high']:+.3f}].",
        f"The frozen-prior distribution-matched refiner reached {frozen.onset_f1:.3f}, so supervised minus frozen-prior is {sup.onset_f1 - frozen.onset_f1:+.3f} by the table and bootstrap median {d_frozen['median']:+.3f} with 95% CI [{d_frozen['ci_low']:+.3f}, {d_frozen['ci_high']:+.3f}].",
    ]
    if d_nop["ci_high"] < 0 and d_frozen["ci_high"] < 0:
        lines.append("Both diffusion refiners beat the supervised refiner with a CI that stays below zero, so the iterative denoising machinery appears to be doing real work in this setup.")
    elif d_nop["ci_low"] <= 0 <= d_nop["ci_high"] or d_frozen["ci_low"] <= 0 <= d_frozen["ci_high"]:
        lines.append("At least one diffusion-vs-supervised interval crosses zero, so this run is not tight enough to claim a meaningful diffusion advantage there.")
    else:
        lines.append("The supervised refiner matches or beats the diffusion rows on this locked test, so the diffusion framing is decorative for onset F1 in this ablation.")
    if frozen.false_positives_per_min < nop.false_positives_per_min:
        lines.append(f"The frozen-prior row still has fewer false positives/min than no-prior diffusion ({frozen.false_positives_per_min:.1f} vs {nop.false_positives_per_min:.1f}), preserving the precision-regularizer interpretation.")
    lines.append(f"Short fragments/min are supervised {sup.short_fragments_per_min:.1f}, no-prior diffusion {nop.short_fragments_per_min:.1f}, and frozen-prior diffusion {frozen.short_fragments_per_min:.1f}.")
    return " ".join(lines)
...
refiner_payload = torch.load(ROOT / "runs/{RUN_NAME}/supervised_refiner.pt", map_location="cpu", weights_only=False)
...
supervised_ckpt = OUT_DIR / "supervised_refiner.pt"
if supervised_ckpt.exists():
    print("reusing supervised refiner checkpoint:", supervised_ckpt)
    refiner_payload = torch_load(supervised_ckpt)
    refiner = SupervisedRefiner(hidden=HIDDEN).to(device).eval()
    refiner.load_state_dict(refiner_payload["model"])
else:
    ...
    refiner, refiner_metrics = train_supervised_refiner(
        baseline,
        train_ds,
        val_ds,
        audio_cfg,
    ...
    print("supervised validation metrics:", refiner_metrics)
...
refiner_eval = maybe_compile(refiner, "supervised refiner eval")
...
variant_order = ["baseline_raw", "supervised_refiner", "no_prior_distmatched", "frozen_prior_distmatched"]
...
write_reload_snippet(ARTIFACT_DIR / "reload_supervised_refiner_eval.py", float(selected_thresholds["supervised_refiner"]))

report = [
    "# Supervised Refiner Ablation",
    "",
    "## Locked Test Results",
    "",
    results[[
...
(ARTIFACT_DIR / "supervised_refiner_ablation_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
...
manifest = {
    ...
    "supervised_checkpoint": str(supervised_ckpt.relative_to(ROOT)),
...
with tarfile.open(bundle_path, "w:gz") as tf:
    for path in [
        ...
        ARTIFACT_DIR / "reload_supervised_refiner_eval.py",
        ARTIFACT_DIR / "supervised_refiner_ablation_report.md",
